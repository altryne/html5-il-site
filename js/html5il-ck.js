angular.module('meetupAPIService',['ngResource']).
	factory('meetupAPIResource',function($resource,$location){
		...
		meetupAPIResource.prototype.getData = function(action,data,callback){
 ...
 * Date: 15/1/13
 * Time: 22:04
 * forked from https://gist.github.com/956897
 */
...
app.config(function($routeProvider,$locationProvider){
	...
	$routeProvider.
		...
		when('/join_the_group',{templateUrl:"/partials/join.html"});
})

var key = '2vhfkjrdi9e2lashik72u7m54h',token;
...
var AppCtrl = function ($scope,$location) {
	...
	$scope.authorize = function(){
		var auth_url = 'https://secure.meetup.com/oauth2/authorize'+
				   '?client_id=' +key +
				   '&response_type=token'+
				   '&redirect_uri=http://html5il.com/auth.html';
				win = new_win(auth_url,'auth_window',400,380);
	}
}
var LoggedinCtrl = function ($scope,meetupAPIResource,$location) {
	$scope.events = [];
	$scope.user = '';
	//get user details
	...
	meetupAPIResource.getData('events',{"page":"20","order":"time","desc":"true","fields":"self"}, function(result){
		console.log(result);
...
//	$http.defaults.headers.post["Content-Type"] = "application/x-www-form-urlencoded";
	...
	$scope.join = function(){
//		$http.post("https://api.meetup.com/2/profile", {"access_token":$.cookie('auth'),"group_id":$scope.group_id,"group_urlname":$scope.group_urlname})
...
var setToken = function(t,expires){
	...
	if(expires){
		...
		var d = new Date(currentDate.getTime() + expires);
		$.cookie('auth', t, { expires: d, path: '/' });
	}
}
